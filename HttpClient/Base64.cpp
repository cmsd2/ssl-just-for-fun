...
/*
input is an array of bytes of length len.
output is an array with at least ((len + 2) / 3) * 4 bytes of space
*/
void Base64Encode(const unsigned char *input, int len, unsigned char *output)
{
	do
	{
		...
		*output++ = base64[((input[0] & 0x3) << 4) | ((input[1] & 0xF0) >> 4)];
